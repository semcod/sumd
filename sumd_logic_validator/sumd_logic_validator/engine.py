...
def is_variable(x: Any) -> bool:
    ...
    if isinstance(x, str) and x:
        return (x[0].isupper() or x == "_") and re.match(r"^[A-Z_][a-zA-Z0-9_]*$", x) is not None
    return False
...
def to_term(x: Any) -> Any:
    ...
    if is_variable(x):
        ...
        return Variable(var_name) if isinstance(x, str) else x
    if isinstance(x, str):
        x = x.strip()
        if x.startswith("\\+"):
            inner_str = x[2:].strip()
...
class HybridPrologEngine:
    ...
    def query(self, query_str: str) -> List[Dict[str, Any]]:
        ...
        if PYSWIP_AVAILABLE:
            try:
                ...
                # Log or handle C library missing issue, then fallback
                pass
    ...
    def _query_subprocess(self, query_str: str) -> List[Dict[str, Any]]:
        ...
        # Convert variables into printing commands
        # For query: ancestor(john, X) -> swipl query
        # We ask Prolog to print result variables in JSON or custom format
        # E.g. findall(X, query(X), L) or similar
        # A simpler way is to construct a short query string that prints output
